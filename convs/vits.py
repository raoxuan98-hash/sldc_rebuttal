...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, num_used_layers=None):
        if num_used_layers is None:
            num_used_layers = self.num_used_layers
        if num_used_layers == 1:
            x = self.forward_features(x, layer_feat=False)
        else:
            layer_feats = self.forward_features(x, layer_feat=True)
            return_feats = []
            if self.global_pool:
                for layer in range(num_used_layers):
                    feat = layer_feats[-layer-1][:, 1:, :].mean(dim=1)
                    return_feats.append(feat)
            else:
                for layer in range(num_used_layers):
                    feat = layer_feats[-layer-1][:, 0]
                    return_feats.append(feat)
            x = torch.cat(return_feats, dim=1)
        x = {'features': x}
        return x
...
@register_model
def vit_base_lora_patch16_224_mae(pretrained=False, lora_rank=4, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    ...
    model = _create_vision_transformer('vit_base_patch16_224_in21k', pretrained=False, **model_kwargs)
    del model.head
    ckpt = load_file("mae_vits/model.safetensors", device='cpu')
    state_dict = model.state_dict()
    state_dict.update(ckpt)
    model.load_state_dict(state_dict)
    del model.norm
